...
def build_analytics_layout() -> html.Div:
    """Build the analytics tab layout (lazy — called on tab select)."""
    logger.info("Building analytics tab layout")
    ...
    return html.Div(children=[
        ...
        html.Div(className="drill-panel", children=[
            ...
            html.Div(className="drill-note", children=[
                "Multi-risk triage aid (not a diagnosis) across four connected "
                "targets — readmission, mortality, high cost and deterioration. "
                "Scores use only features known at or before discharge — no "
                "future data leaks in. Best model per risk by PR-AUC. Weaker "
...
# ===========================================================================
# DATA INTAKE TAB  (formerly upload_app.py)
# ===========================================================================
...
def train_uploaded(n_clicks, data_json, target, features):
    ...
    if not n_clicks or data_json is None:
        return None
    if not target:
        return html.Div("Pick a target column.", className="drill-note")
    if not features:
    ...
    if result.error:
        return html.Div(f"Could not train: {result.error}",
                        className="drill-note", style={"color": RED})
