...
<style>
  ...
  #react-entry-point,._dash-loading{background:#12140F;}
  .page,[class*="page"]{background:#12140F!important;color:#EAEEE6;min-height:100vh;}
  .dash-table-container .dash-spreadsheet td{background:#181B15!important;color:#EAEEE6!important;}
  input,textarea,select{background:#181B15!important;color:#EAEEE6!important;border-color:#2B2F28!important;}
...
def _oe_badge(oe):
    ...
    return html.Span(f"O/E {oe:.2f}", style={
        "fontSize": "11px",
        "background": "#14301F" if good else "#301414",
...
def build_grid(n_clicks, data_json):
    ...
    disclosure = html.Div(style={
        "background": "#2A2410", "border": f"1px solid {AMBER}",
...
def open_dept(clicks, scored_json):
    ...
    legend = html.Details(style={
        "background": "#181B15", "border": f"1px solid {GRID}",
    ...
    table = dash_table.DataTable(
        ...
        style_data_conditional=[
            {"if": {"row_index": "odd"}, "backgroundColor": "#181B15"}])
